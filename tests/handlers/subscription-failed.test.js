import { handler } from '../../src/handlers/subscription-failed';
import { failedMessagesService } from '../../src/services/failed-messages-service';

jest.mock('../services/failed-messages-service');

describe('subscription-failed handler', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });

  it('should store failed messages and return batch item failures', async () => {
    const event = {
      Records: [
        {
          messageId: '1',
  ...
  it('should return batch item failures for rejected promises', async () => {
    const event = {
      Records: [
        {
          messageId: '1',
